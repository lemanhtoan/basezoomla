 ...
 */
class BlogModelBlogs extends JModelList
{
...
if (empty($list['direction']))
{
    ...
    $query->join('LEFT', '#__users AS uc ON uc.id=a.checked_out');
		...
		// Join over the category 'categories'
		$query->select('categories.title AS categories_title');
		$query->join('LEFT', '#__categories AS categories ON categories.id = a.categories');
		// Join over the created by field 'created_by'
		$query->join('LEFT', '#__users AS created_by ON created_by.id = a.created_by');

		
if (!JFactory::getUser()->authorise('core.edit.state', 'com_blog'))
{
	$query->where('a.state = 1');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.name LIKE '.$search.'  OR  a.description LIKE '.$search.'  OR  a.content LIKE '.$search.' )');
			}
		}
		...
		if ($orderCol && $orderDirn)
		{
			...
			$query->order($db->escape('a.ordering' . ' ' . 'ASC')); //id
            //$query->order($db->escape('a.mode' . ' ' . 'DESC'));
		}
		...
		foreach($items as $item){
			...
			if ( isset($item->categories) ) {
				...
				// Get the title of that particular template
					$title = BlogFrontendHelper::getCategoryNameByCategoryId($item->categories);

					// Finally replace the data object with proper information
					$item->categories = !empty($title) ? $title : $item->categories;
				}
					$item->mode = JText::_('COM_BLOG_BLOGS_MODE_OPTION_' . strtoupper($item->mode));
}
