...
class ContactModelContact extends JModelForm
{
	...
	public static function getUserInfomations($user_id) {
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query
			->select('u.name, CONCAT(u.street, ", ", u.zipcode, " ", u.city, ", ", (select country_name from #__virtuemart_countries where country_3_code = u.country)) address, u.phone, u.email')
			->from('#__users u')
			->where('u.id = '.$user_id);
		$db->setQuery($query);
