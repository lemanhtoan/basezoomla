...
//add class filesystem upload image
...
class UsersControllerProfile extends UsersController
{
	...
	public function save()
	{
        ...
        $uploadDir = JPATH_BASE . '/images/user/';
		...
		if( !empty($files) && isset($files['image']) && (strlen($files['image']['name']) > 0) ) { 
            $file_image = $files['image'];
            $tmp_dir = $file_image['tmp_name'];
            $dest_dir = $uploadDir;
            $file_name_ext = microtime(true) . '_' . $file_image['name'];
            if( JFile::upload($tmp_dir,$dest_dir . $file_name_ext) )
            {
                $filePath = $uploadDir . $file_name_ext;                
            }
            
            $data['image'] = 'images/user/' . $file_name_ext;
		...
		switch ($this->getTask())
		{
			case 'apply':
                ...
                $edit_param = '&edit=done';
            
				if (!$redirect)
				{
					$redirect = 'index.php?option=com_users&view=profile&layout=edit&hidemainmenu=1'.$edit_param;
				}
			...
			default:
				...
				$this->setRedirect(
					JRoute::_(
						($redirect = $app->getUserState('com_users.edit.profile.redirect')) ? $redirect : 'index.php?option=com_users&view=profile&user_id=' . $return.$edit_param,
						false
					)
				);
				break;
