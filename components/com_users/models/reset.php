...
class UsersModelReset extends JModelForm
{
    ...
    public function AjaxProcessResetRequest($data)
	{ 
		$config = JFactory::getConfig();
		...
		// Assemble the password reset confirmation link.
		$mode =  1; //$config->get('force_ssl', 0) == 2 ? 1 : (-1);
		//$itemid = UsersHelperRoute::getLoginRoute();
        //var_dump($itemid); die;
        $itemid = $itemid !== null ? '&Itemid=' . $itemid : '';
		$link = 'index.php?option=com_users&view=reset&layout=confirm&token=' . $token . $itemid . '&show=1';
		...
		$data['mailfrom'] = $config->get('mailfrom');
		$data['sitename'] = $config->get('sitename');
		$link_ssl = JRoute::_($link, false, $mode);
        $link_nonssl= str_replace("https","http",$link_ssl);
		$data['link_html'] = JRoute::_($link, true, $mode);
		$data['token'] = $token;
