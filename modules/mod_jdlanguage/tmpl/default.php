...
<?php foreach($languages as $language):?>
    ...
    <?php if (in_array('flag', $style)):
			if (file_exists(JPATH_SITE.'/media/com_jdiction/images/flags/'.$language->image.'.png')) {
				$flagurl = 'media/com_jdiction/images/flags/'.$language->image.'.png';
			} else {
				$flagurl = JHTML::_('image', 'mod_languages/' . $language->image . '.gif', $language->title_native, array('title' => $language->title_native), true, true);
			}
			?>
    style="background-image: url(<?php echo JRoute::_($flagurl); ?>);"
    <?php endif; ?>><?php
      if (in_array('text', $style)):
        echo $language->title_native;
      elseif (in_array('tag', $style)):
