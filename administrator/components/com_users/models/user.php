...
class UsersModelUser extends JModelAdmin
{
	...
	{   
        // subcribe
        // update and not check
        if($data['subscribe'] == 0) {
            // find and delete with user_id: subcribe table
            $email_add = $data['email'];
            //check exist - not insert into subcribe
            $db = JFactory::getDbo(); 
            $query = $db->getQuery(true);
            $query->select('*');
            $query->from($db->quoteName('#__subscribe'));
            $query->where($db->quoteName('email') . ' = '. $db->quote($email_add));
            $db->setQuery($query);
            ...
            if(count($results) >= 1) {                
                ...
                $query = $db->getQuery(true); 
                $conditions = array(
                    $db->quoteName('email') . ' = ' . $db->quote($email_add)
                );                 
                $query->delete($db->quoteName('#__subscribe'));
        ...
        // update change value subcribe into user
        ...
        if($data['subscribe'] == 1) { 
            ...
            if(count($results) < 1) {
                ...
                $values = array('"' . $email_add . '"', '"' . date("Y-m-d") . '"', '"' . 1 . '"', '"' . $data['language_user'] . '"');
        ...
        if( ($data['id'] != 289) && ($data['groups'][1] != 7) ){
        ...
        //auto email sent to User with log in credentials => pasword = password 2 (not auto genarate)
        
        
		// begin custom sent email to user   
		...
		//$data['mailfrom']	= $data['email'];
        
		$data['sitename']	= "Baselli";
        ...
        switch ($data['country']) {
			case "CHE": 
				$emailSubject = JText::sprintf("New User Details Baselli Switzerland");
                $emailBody = JText::sprintf(  "Dear " .  $data['name'] . ", " . "<br/>" .  " You have been added as a User to Baselli by an Administrator. You can now visit Basseli (" . 
			         $data['siteurl'] . '). Email: ' . $data['email'] . ', Your password (you can change password after fisrt login) ' . $data['password2'] . '.'
		          );
				break;
			case "DEU":
				$emailSubject = JText::sprintf("New User Details Baselli Germany");
                $emailBody = JText::sprintf(  "Liebe " .  $data['name'] . ", " . "<br/>" .  " Thank you for registering as a customer of Baselli. Sie würden vom Administrator als Benutzer hinzugefügt. Sie können Baselli nun besuchen. (" . 
			         $data['siteurl'] . '). Email: ' . $data['email'] . ', Unser Passwort (Sie können das Passwort beim ersten Besuch ändern) ' . $data['password2'] . '.'
		          );
				break;
			case "AUT":
				$emailSubject = JText::sprintf("New User Details at Baselli Austria");
                $emailBody = JText::sprintf(  "Dear " .  $data['name'] . ", " . "<br/>" .  " Thank you for registering as a customer of Baselli.You have been added as a User to Baselli by an Administrator.  You can now visit Baselli (" . 
			         $data['siteurl'] . '). Email: ' . $data['email'] . ', Your password (you can change password after fisrt login) ' . $data['password2'] . '.'
		          );
				break;
            case "FRA":
				$emailSubject = JText::sprintf("New User Details at Baselli France");
                $emailBody = JText::sprintf(  "Cher " .  $data['name'] . ", " . "<br/>" .  " Thank you for registering as a customer of Baselli. Ils seraient ajoutés par l'administrateur en tant qu'utilisateur. Vous pouvez maintenant visiter Baselli (" . 
			         $data['siteurl'] . '). Email: ' . $data['email'] . ', La nostra parola d\'ordine (è possibile modificare la password alla prima visita) ' . $data['password2'] . '.'
		          );
				break;
            case "ITA":
				$emailSubject = JText::sprintf("New User Details at Baselli Italy");
                $emailBody = JText::sprintf(  "Caro " .  $data['name'] . ", " . "<br/>" .  " Thank you for registering as a customer of Baselli. Sarebbero aggiunti dall'amministratore come utente. A questo punto è possibile visitare Baselli (" . 
			         $data['siteurl'] . '). E-mail: ' . $data['email'] . ', Notre mot de passe (vous pouvez changer le mot de passe lors de la première visite) ' . $data['password2'] . '.'
		          );
				break;
		}
		...
		$mailer->setSender(array($user->email, $data['sitename']));
    ...
    public function getModuleParams() {
     ...
     public function parepareMailContent($email, $lang_code) {
        ...
        <div class="wrapper" style="font-size: 14px; color: #333; line-height: 1.42857143; font-weight: 400;">
        	<div id="header">
        		<div class="container" style="background: #05081b; height: 100px; width: 610px; margin: auto; position: relative;">&gt;
        			<table>
        				<tbody>
        					<tr>
        						<td><a id="logo" style="display: inline-block; position: absolute; bottom: 0; left: 20px;" href="http://baselli.ch"> <img src="http://baselli.ch/templates/baselli/images/order_email_logo.png" alt="" width="232" height="45" /> </a></td>
        						<td style="width: 100%; text-align: right; padding-right: 10px;"><a style="display: inline-block; text-decoration: none; text-transform: uppercase; color: white; right: 10px; bottom: 15px; position: absolute; font-size: 15px; vertical-align: -16px; padding-top: 40px;" href="#">&nbsp;</a></td>
        					</tr>
        				</tbody>
        			</table>
        		</div>
        	...
        	<div class="container" style="width: 610px; margin: auto; position: relative;">
        		<div style="clear: both; float: left; width: 610px; font-size: 20px; margin-top: 35px; color: #333333; line-height: 24px; padding-bottom: 30px;">CONTENT__ADD</div>
        			<div style="clear: both;"></div>
        			<div id="footer">
        				<div class="container" style="width: 610px; margin: auto; position: relative; height: 75px; background: #05081b; clear: both; padding-top: 10px;">
        					<div class="cp" style="float: left; padding: 10px 10px; color: white;">
        						<p style="margin: 0; font-size: 13px; color: #cccccc;">Copyright &copy;2015 BASELLI. All Rights Reserved</p>
        						<p style="margin: 0; font-size: 13px;"> </p>
        					</div>
        					<p class="social" style="margin: 0; font-size: 13px; float: right; padding: 10px; line-height: 55px;"><a href="http://baselli.ch"><img src="http://baselli.ch/templates/baselli/images/order_email_facebook.png" alt="" /></a> <a href="http://baselli.ch"><img src="http://baselli.ch/templates/baselli/images/order_email_twitter.png" alt="" /></a></p>
        				</div>
        			</div>
        	</div>
        </div>                
	...
	public function block(&$pks, $value = 1)
	{
        ...
        if($userInfo->block == 1) {
            ...
            //CHE Switzerland / DEU Germany / AUT Austria / FRA France / ITA Italy
            switch ($userInfo->language_user) {
    			case "en-GB": 
    				$emailSubject = JText::sprintf("Thank you for register at Baselli");
                    $mail_body = JText::sprintf(  "Dear " .  $userInfo->name . ", " . "<br/>" .  " Thank you for registering as a customer of Baselli. Your contact information were 
                        confirmed and your account was activated.  You can now visit Basseli website (" . 
    			         $data['siteurl'] . 'en/). Website now and login with your email: ' . $userInfo->email . '.<br>'
    		          );
    				break;
    			case "de-DE":
    				$emailSubject = JText::sprintf("Thank you for register at Baselli");
                    $mail_body = JText::sprintf(  "Dear " .  $userInfo->name . ", " . "<br/>" .  " Thank you for registering as a customer of Baselli. Your contact information were 
                        confirmed and your account was activated.  You can now visit Basseli website (" . 
    			         $data['siteurl'] . 'de/). Website now and login with your email: ' . $userInfo->email . '.<br>'
    		          );
    				break;
    			case "fr-FR":
    				$emailSubject = JText::sprintf("Thank you for register at Baselli");
                    $mail_body = JText::sprintf(  "Dear " .  $userInfo->name . ", " . "<br/>" .  " Thank you for registering as a customer of Baselli. Your contact information were 
                        confirmed and your account was activated.  You can now visit Basseli website (" . 
    			         $data['siteurl'] . 'fr/). Website now and login with your email: ' . $userInfo->email . '.<br>'
    		          );
    				break;
                case "it-IT":
    				$emailSubject = JText::sprintf("Thank you for register at Baselli");
                    $mail_body = JText::sprintf(  "Dear " .  $userInfo->name . ", " . "<br/>" .  " Thank you for registering as a customer of Baselli. Your contact information were 
                        confirmed and your account was activated.  You can now visit Basseli website (" . 
    			         $data['siteurl'] . 'it/). Website now and login with your email: ' . $userInfo->email . '.<br>'
    		          );
    				break;
    		}
            ...
            $mail_body_sent = $this->parepareMailContent($userInfo->email, $userInfo->language_user);
            
            $mail_body_sent = str_replace( "CONTENT__ADD",   $mail_body, $mail_body_sent);
