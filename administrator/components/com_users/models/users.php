...
class UsersModelUsers extends JModelList
{
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'block', 'user_list.block',
				'sendEmail', 'user_list.sendEmail',
				'registerDate', 'user_list.registerDate',
				'lastvisitDate', 'user_list.lastvisitDate',
	...
	protected function populateState($ordering = null, $direction = null)
	{
        ...
        $user_retail_group = $this->getUserStateFromRequest($this->context . '.filter.user_retail_group', 'user_retail_group');
		$this->setState('filter.user_retail_group', $user_retail_group);
		...
		parent::populateState('user_list.name', 'asc');
	}
	...
	protected function getStoreId($id = '')
	{
        ...
        $id .= ':' . $this->getState('filter.user_retail_group');
		...
		if (empty($this->cache[$store]))
		{
			...
			foreach ($items as &$item)
			{
				if (isset($userGroups[$item->id]))
				{
                    ...
                    if(count($item->group_names) >= 2){
                        $item->group_names = array($item->group_names[1]);
                    } else{
                        $item->group_names = $item->group_names;
                    }
				}
			...
			if ( $sysKey != -1 ) unset($items[$sysKey]);
	...
	protected function getListQuery()
	{
        ...
        //echo $this->getState('filter.user_retail_group');
        if( ($this->getState('filter.user_retail_group') != null) || ($this->getState('filter.user_retail_group') != "") ) {
            $query->where($db->quoteName('a.user_retail_group') . ' = '. " '" . $this->getState('filter.user_retail_group') . " '" . ' ');
        }
		...
		if ($groupId || isset($groups) || 1)
		{
			$query->join('LEFT', '#__user_usergroup_map AS map2 ON map2.user_id = a.id')
                ->group($db->quoteName(array('a.id', 'ug.id')));
			if ($groupId)
			{
        ...
        $query->join('LEFT', '#__usergroups AS ug ON ug.id = map2.group_id');
		// Add the list ordering clause.
		$query->order('ug.title ASC');

        $order = ( isset($_REQUEST['list']) && isset($_REQUEST['list']['fullordering']) )  ? $_REQUEST['list']['fullordering'] : " user_list.title asc ";
	...
	{
		$db    = JFactory::getDbo();
		$query = "SELECT * FROM " . $db->quoteName('#__usergroups') . " ug left join " .
