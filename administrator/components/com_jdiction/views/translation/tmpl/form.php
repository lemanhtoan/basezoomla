...
<script type="text/javascript">
  ...
  jQuery(function() {
    ...
    var ed;
     ...
     jQuery(function() {
        //get param language url
        var url_full = window.location.href;
        var param_lang = url_full.split('#');
        
        //remove active first
        jQuery("#language-tabsContent").children("div").removeClass('active');
        $items = jQuery("#language-tabsContent").children("div");
        jQuery($items).each(function(i, item) {
            var $item = jQuery(item);        
            if($item.attr("id") == param_lang[1] )
            {
        ...
        //process tab
        jQuery("#language-tabsTabs").children("li").removeClass('active');
        
        $tab_items = jQuery("#language-tabsTabs").children("li");
        jQuery($tab_items).each(function(i, tab_item) {
            var $tab_item = jQuery(tab_item); 
            $tab_item.html();
            var n = $tab_item.html().indexOf(param_lang[1]);
            if (n != -1){
