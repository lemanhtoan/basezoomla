...
jQuery(function() {
  var tbl;
  if (typeof jDictionTable === 'undefined') {
    return;
  }

  tbl = jQuery(jDictionTable.tableselector);
  ...
  if (jDictionTable.columnselector != '' && jDictionTable.columnselector > -1) {
    position = jDictionTable.columnselector;
  } else {
  ...
  jQuery.each(tblBody[0].children, function(line, item){
    ...
    if (row['link'] != '') {
      ...
      jQuery.each(row['status'], function(lang, status) {
        ...
        var x = jQuery('<a href="'+row[lang]['link'] + '" class="btn btn-micro"><span rel="tooltip" data-original-title="'+lang+'" style="background: url(../media/mod_languages/images/'+lang+'.gif) no-repeat center center; width: 26px; height: 16px; display: inline-block;"><img src="components/com_jdiction/assets/icon-status-26-'+status+'.png" /></span></a>');
