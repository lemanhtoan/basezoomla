...
class jDictionModelCheck extends JModelLegacy {
	...
	public function checkPlugin() {
		...
		$db->setQuery($query);
		$languagefilter = $db->loadObject();

		if ($languagefilter->enabled < 1) {
			$result[] = $this->text(2, 'Joomla Languagefilter Plugin is not enabled');
		} else {
			$result[] = $this->text(0, 'Joomla Languagefilter Plugin is enabled');
		}
		...
		$query->where($query->quoteName('type') . '=' . $query->quote('plugin'));
		$query->where($query->quoteName('folder') . '=' . $query->quote('system'));
		$query->where($query->quoteName('ordering') . '<'.$languagefilter->ordering);

		$db->setQuery($query);
		$list = $db->loadRowList();
		if ($list && count($list) > 0) {
			$result[] = $this->text(1, "Joomla Languagefilter Plugin is not the first loaded Plugin.");
		}
		...
		if ($list && count($list) > 1) {
			$result[] = $this->text(1, "There are ".(count($list)-1)." plugins having the same loading postion as the Joomla Languagefilter Plugin.");
		}
		...
		if ($plugin->ordering - $languagefilter->ordering > 1) {
			$result[] = $this->text(1, "JDiction Plugin is not directly loaded after the Joomla Languagefilter Plugin");
		}
		...
		$query->where($query->quoteName('folder') . '=' . $query->quote('system'));
		$query->where($query->quoteName('ordering') . '>'.$languagefilter->ordering);
		$query->where($query->quoteName('ordering') . '<'.$plugin->ordering);

		$db->setQuery($query);
		$list = $db->loadRowList();
		if ($list && count($list) > 0) {
			$result[] = $this->text(1, "There are plugin(s) loaded between the Joomla Languagefilter Plugin and the jDiction plugin.");
		}
	...
	public function checkLanguage() {
		...
		/** todo Check Tag incl. upper and lowercase
		 * todo show installed components
		$query->clear();
		$query->select('lang_id');
		$query->from('#__languages');
		$query->where($query->quoteName('access') . '=' . $query->quote('0'));
