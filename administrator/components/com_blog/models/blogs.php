...
class BlogModelBlogs extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
		...
		//Filtering mode
		$this->setState('filter.mode', $app->getUserStateFromRequest($this->context.'.filter.mode', 'filter_mode', '', 'string'));
    ...
    protected function getListQuery() {
		...
		$query->select('categories.title AS categories');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.categories LIKE '.$search.'  OR  a.mode LIKE '.$search.'  OR  a.name LIKE '.$search.'  OR  a.image LIKE '.$search.'  OR  a.description LIKE '.$search.'  OR  a.content LIKE '.$search.'  OR  a.language LIKE '.$search.'  OR  a.created_date LIKE '.$search.' )');
            }
        }
        ...
        if ($orderCol && $orderDirn) {
            ...
            $query->order($db->escape(' ' . 'a.id' . ' ' . 'DESC')); // add condition other sort
        }
        
        //echo $query->dump(); die("dd");
        return $query;
    }
