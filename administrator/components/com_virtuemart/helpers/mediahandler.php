...
class VmMediaHandler {
	...
	function uploadFile($urlfolder,$overwrite = false){
		...
		$extension = strtolower(JFile::getExt($table->file_url));
        $item = 0;
        foreach($media['tmp_name'] as $key => $tmp_name ){
			$file_name = time() . "_" . $key.$media['name'][$key];
            $file_size =$media['size'][$key];                      

			$file_tmp =$media['tmp_name'][$key];
			$file_type=$media['type'][$key];	
			if($file_size > 2097152){
				$errors[]='File size must be less than 2 MB';
			}	
            
            // process upload and insert zfg_virtuemart_medias
            $db = JFactory::getDBO();
            if ($_REQUEST['controller'] === 'category') { // file_type = category	    
                $desired= dirname(JPATH_BASE) . "/images/stories/virtuemart/category/" . $file_name;
                $desired_dir="images/stories/virtuemart/category/" . $file_name;
                $query="INSERT INTO zfg_virtuemart_medias(`file_title`,`file_mimetype`,`file_type`,`file_url`) VALUES('$file_name','$file_type','category','$desired_dir'); ";  
                              
            } else {
                $desired= dirname(JPATH_BASE) . "/images/stories/virtuemart/product/" . $file_name;
                $desired_dir="images/stories/virtuemart/product/" . $file_name;
                $query="INSERT INTO zfg_virtuemart_medias(`file_title`,`file_mimetype`,`file_type`,`file_url`) VALUES('$file_name','$file_type','product','$desired_dir'); ";
            }

			if(empty($errors)==true){				
				if(is_dir("$desired".$file_name)==false){
					move_uploaded_file($tmp_name,$desired);
				}else{								
					$new_dir="$desired".$file_name.time();
				    rename($file_tmp,$new_dir) ;				
				}
                $item++;
    			$db->setQuery($query);
                $db->execute();                
                // insert into category media
                $vituare_id = $db->insertid();             
			}else{
					print_r($errors);
			}
		}
        ...
        if ($_REQUEST['controller'] === 'category') { // file_type = category
            $dba = JFactory::getDBO();       
            $query_cateMedia = $dba->getQuery(true);      
            $query_cateMedia->clear();       
            $columns = array('virtuemart_category_id', 'virtuemart_media_id', 'ordering');             
            $values = array($_REQUEST['virtuemart_category_id'], $vituare_id , 1);             
            $query_cateMedia
                ->insert($dba->quoteName('#__virtuemart_category_medias'))
                ->columns($dba->quoteName($columns))
                ->values(implode(',', $values));
                
            $dba->setQuery($query_cateMedia);
             if ($dba->query() === FALSE) {
                echo 'error';
             } else {
                    return $file_name; //true;//exit; //die("D");
             }
        }      
                
        $_SESSION['itemUp'] = $item;
	...
	private function addHiddenByType(){
        ...
        $db_get_max_proId = JFactory::getDBO();
        $query_proId="SELECT MAX(virtuemart_product_id) AS 'maxId' FROM zfg_virtuemart_products_en_gb";
        $db_get_max_proId->setQuery($query_proId);
        $results_proId = $db_get_max_proId->loadObjectList();
        $highest_proId = $results_proId[0]->maxId;
        
        //var_dump($highest_proId); die;
        
        $db = JFactory::getDBO();
        $query="SELECT MAX(virtuemart_media_id) AS 'maxId' FROM zfg_virtuemart_medias";
        $db->setQuery($query);
        $results = $db->loadObjectList();
        $highest_id = $results[0]->maxId;
        


        $db1 = JFactory::getDBO();
        $query1="SELECT MAX(id) AS 'maxId1' FROM zfg_virtuemart_product_medias";
        $db1->setQuery($query1);
        $results1 = $db1->loadObjectList();
        $highest_id1 = $results1[0]->maxId1;        
        
        if(is_int($_REQUEST['virtuemart_product_id'])) {
            $idProduct = $_REQUEST['virtuemart_product_id'];
        } else {
            $idProduct = $_REQUEST['virtuemart_product_id'][0]; 
        }
        
        // echo (int)$idProduct . ' / ' . (int)$highest_proId ; die;
        // echo "<pre>"; var_dump($_REQUEST) . '<hr>';
        // echo $_SESSION['itemUp'] . '<hr>';

        //if( ( (int)$idProduct != 1 )  && ( (int)$idProduct ) >= (int)$highest_proId ) {  //var_dump((int)$idProduct); echo "<br>" . var_dump($idProduct) . '<hr>'; die("111");
        if ($_REQUEST["task"] == "edit" && $_REQUEST["controller"] == "product" && $_REQUEST["layout"] == "product_edit" && ($_SESSION['itemUp']>0)) {
            $db_count = JFactory::getDbo();
            $qr_count = "SELECT * , COUNT(*) as count
                        FROM zfg_virtuemart_product_medias
                        WHERE virtuemart_product_id = '" . (int)$idProduct . " '
                        GROUP BY virtuemart_product_id                            
                        "; //HAVING COUNT(*) > 1
            
            $db_count->setQuery($qr_count);
            $rs_count = $db_count->loadObjectList();
            $countv = (int)$rs_count[0]->count;
                                
            for ($x = 1; $x <= $_SESSION['itemUp']; $x++) {
                $db = JFactory::getDBO();            
    			$query="INSERT INTO zfg_virtuemart_product_medias(`id`,`virtuemart_product_id`,`virtuemart_media_id`,`ordering`) 
                    VALUES(' " . ($highest_id1 + $x) . " ',' " . $idProduct . " ',' " . ($highest_id + $x - $_SESSION['itemUp']) . " ',' " . $x . "'); ";
    			$db->setQuery($query);
                $db->execute();
                //echo $query . '<hr>';
        ...
        $query12="DELETE FROM zfg_virtuemart_product_medias WHERE virtuemart_media_id = 73 AND ordering = 1";
...
//]]>
	...
	function displayImage($image ,$key) {
        ...
        if ((isset($image->file_url)) && strlen($image->file_url) == 34 ){
            $db = JFactory::getDBO();
            $query="select virtuemart_media_id,file_title,file_url   from zfg_virtuemart_medias ORDER BY virtuemart_media_id DESC LIMIT " . $_SESSION['itemUp'];
            $db->setQuery($query);
            $results = $db->loadObjectList();
            ...
            foreach($results as $item) {
               $string .=   '<div  class="vm_thumb_image"><input type="hidden" value="'.$item->virtuemart_media_id.'" name="virtuemart_media_id[]">
        			<input class="ordering" type="hidden" name="mediaordering['.$item->virtuemart_media_id.']" value="'.$key.'">
   		     <a class="vm_thumb" rel="group1" title ="'.$item->file_title.'" href="'.JURI::root(true).'/'.$item->file_url.'" >
        		<img src="' . JURI::root(true).'/'.$item->file_url . '" alt="' . $item->file_title . '"  />
		...
		<img src="' . JURI::root(true).'/'.$image->file_url . '" alt="' . $image->file_title . '"  />
		</a><div class="vmicon vmicon-16-remove" title="'.vmText::_('COM_VIRTUEMART_IMAGE_REMOVE').'"></div><div class="edit-24-grey" title="'.vmText::_('COM_VIRTUEMART_IMAGE_EDIT_INFO').'"></div></div>';
		} else {
	...
	public function displayFileHandler(){
		...
		$html .= vmText::_('COM_VIRTUEMART_FILE_UPLOAD').' <input type="file" name="upload[]" id="upload" size="50" class="inputbox" multiple/><br />';
		...
		$html .= ' <table class="adminform" style="border: 0"> ';
