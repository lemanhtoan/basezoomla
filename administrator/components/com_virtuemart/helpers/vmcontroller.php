...
class VmController extends JControllerLegacy{
	...
	*/
	...
	function getLanguageListExceptDefault()
	{
		$db = JFactory::getDbo();
	...
	function save($data = 0){
		...
		if ( $this->_cname == 'country'  )
		{
			$db = JFactory::getDbo();
			//added by truong12a6209@gmail.com
			$request = vRequest::getRequest();

			$languages = $this->getLanguageListExceptDefault();
			foreach ( $languages as $language )
			{

				$key = 'country_name_' . $language->lang_id;
				$xml = '<?xml version="1.0"?><jDiction><country_name><![CDATA[]]></country_name></jDiction>';
				if ( isset($request[$key]) )
				{
					$xml = '<?xml version="1.0"?><jDiction><country_name><![CDATA[' . $request[$key] .']]></country_name></jDiction>';
				}
				$referTable = '#__languages';
				//check is existed
				$query = $db->getQuery(true);
				$query->select( '*' );
				$query->from( $db->quoteName('#__jd_store') );
				$query->where( $db->quoteName('referenceTable') . ' = "' . $referTable .'" AND idLang = ' . intval($language->lang_id) . ' AND idReference = ' . intval($id) . ' AND referenceOption = "com_languages"');
				$db->setQuery($query);
				$languageCheck = $db->loadObject();
				if ( is_object($languageCheck)  && property_exists($languageCheck, 'idJdStore') )
				{
					//existed
					$query = $db->getQuery(true);
					$fields = array(
					    $db->quoteName('value') . ' = ' . $db->quote($xml),
					    $db->quoteName('modified') . ' = "' . $db->quote(date("Y-m-d H:i:s")) . '"',
					);

					// Conditions for which records should be updated.
					$conditions = array(
					    $db->quoteName('idLang') . ' = ' . intval($language->lang_id), 
					    $db->quoteName('idReference') . ' = ' . intval($id),
					    $db->quoteName('referenceTable') . ' = "' . $referTable . '"',
					    $db->quoteName('referenceOption') . ' = "com_languages"',
					);
				...
				else 
				{
					//insert new
					$query = $db->getQuery(true);
					$columns = array('idLang', 'idReference', 'referenceTable', 'value', 'modified_by', 'referenceOption');
					$values = array(intval($language->lang_id), intval($id), $db->quote($referTable), $db->quote($xml), $db->quote(date("Y-m-d H:i:s")), $db->quote('com_languages'));//echo "<pre>"; var_dump($values);die;
			...
			//echo "<pre>"; var_dump(vRequest::getRequest());die;	
		}
