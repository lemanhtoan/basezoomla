...
class VirtueMartModelCategory extends VmModel {
	...
	*/
    ...
    public function store(&$data) {
		...
		//Edit	
		if($data['virtuemart_category_id'] != 0){
			$sql = "SELECT
						ordering
					FROM
			...
			$db->setQuery($sql);
			$data['ordering'] =(int)$db->loadResult();
		}
		//Add new
		else{
			$sql = " SELECT MAX(ordering) FROM #__virtuemart_categories ";
			$db->setQuery($sql);
			$data['ordering'] =(int)$db->loadResult() + 1;
		}
		
		//var_dump($data);die("mot");
	...
	*/
	...
	function getTop2Image($images_id) {
		$db = JFactory::getDbo();
		$query = 'SELECT `file_url` FROM `#__virtuemart_medias` WHERE `virtuemart_media_id` IN (' . implode(',', $images_id) . ')  ORDER BY virtuemart_media_id ASC LIMIT 2';
		$db->setQuery($query);
