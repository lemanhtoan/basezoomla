...
class VirtueMartModelOrders extends VmModel {
	...
	public function getOrder($virtuemart_order_id){
		...
		$q = "SELECT  o.*,u.*,o.created_on  as order_created_on,
				s.order_status_name
			...
			WHERE o.virtuemart_order_id=".$virtuemart_order_id;//echo str_replace('#__', 'zfg_', $q);die;
		$db->setQuery($q);
...
// 		vmdebug('getOrdersList');
		...
		if($user->authorise('core.admin','com_virtuemart')){
			...
			if(!empty($virtuemart_vendor_id)){
				...
				//$uid = $user->id;
				//$where[]= ' (o.virtuemart_vendor_id = '.$virtuemart_vendor_id.' OR u.virtuemart_user_id = ' . (int)$uid.') '; //commented
				$uid = 0;
			} else {
				//We map here as fallback to vendor 1.
				//$where[]= ' u.virtuemart_user_id = ' . (int)$uid; //commented
...
//echo $select,$from,$whereString,'',$ordering;die;
