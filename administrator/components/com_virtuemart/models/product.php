...
// 		vmdebug('my $this->searchcustoms ',$this->searchcustoms);
		...
		if ($this->search_type != '') {
			...
			switch ($this->search_type) {
				case 'parent':
					...
					//die("TVX mac dinh cho show theo parent");
					break;
				case 'product':
		...
		//TVX added 21/10/2015 mac dinh cho show theo parent
		$vci = JRequest::getVar('virtuemart_category_id');
		if($this->search_type == '' && $vci > 0){
			//var_dump($vci);die("3 case se vao");
			
			$where[] = 'p.`product_parent_id` = "0"';
		}
		$cur_url = substr_replace(JURI::root(),'',-1) . $_SERVER['REQUEST_URI'];
		$end_url = trim(substr($cur_url, -44, 44));
		
		if($end_url == 'index.php?option=com_virtuemart&view=product'){
			$where[] = 'p.`product_parent_id` = "0"';
		}
		//TVX end added 21/10/2015
...
// 			$joinLang = false;
	...
	public function getProductSingle ($virtuemart_product_id = NULL, $front = TRUE, $quantity = 1, $withParent=false,$virtuemart_shoppergroup_ids=0) {
		...
		if (!empty($this->_id)) {
			...
			$app = JFactory::getApplication();
			// Check if have child product
			$childProductsId = $this->getChildProductId($virtuemart_product_id);
			$haveChildProducts = count($childProductsId);
			if($haveChildProducts > 0) {
				if($app->isSite()) {
					$product->child = array();
					foreach($childProductsId as $productId) {
						$product->child[] = $this->getProductSingle($productId, TRUE, $quantity = 1, $withParent=true);
					}
					$product->child_id = $childProductsId;
				}
			} else {
				$product->child = 0;
			}

			// Check if have parent product
			if($app->isSite()) {
				if($product->product_parent_id > 0) {
					$product->product_parent_name = $this->getParentProductName($product->product_parent_id);
					$product->product_parent_info = $this->getParentProductInfo($product->product_parent_id);
				}
			}
	...
	public function getProductsInCategory ($categoryId) {
		...
		$this->products = $this->getProducts ($ids); //echo '<pre>'; var_dump($this->products); die;
	...
	public function getProducts ($productIds, $front = TRUE, $withCalc = TRUE, $onlyPublished = TRUE, $single = FALSE) {
        ...
        $orderby = "ordering"; //change this to whatever key you want from the array 
        
        array_multisort($sortArray[$orderby],SORT_ASC,$products);         
        //echo "<pre>"; var_dump($products); die("ddd");        
        
        //echo "<pre>"; var_dump($products); die;
        /*foreach($products as $item){
            echo "<pre>";
            var_dump($item->virtuemart_product_id);
        }
        die("d");*/
		return $products;
	}
...
function lowStockWarningEmail($virtuemart_product_id) {
	...
	public function getChildProductId($parent_id) {
		$db = JFactory::getDbo();
		//die("TVX");
		
		
		$query = 'SELECT `virtuemart_product_id` childs_id FROM `#__virtuemart_products` WHERE `product_parent_id` = ' . $parent_id . ' AND published = 1 ORDER BY `pordering` ASC';
		$db->setQuery($query);
		return $db->loadColumn();
	}

	public function getParentProductName($parent_product_id) {
		$lang = JFactory::getLanguage();
		$current_lang = strtolower(str_replace('-', '_', $lang->getTag()));

		$db = JFactory::getDbo();
		$query = "SELECT `product_name` FROM `#__virtuemart_products_$current_lang` WHERE `virtuemart_product_id` = " . $parent_product_id;
		$db->setQuery($query);
		return $db->loadRow();
	}

	public function getParentProductInfo($parent_product_id) {
		$db = JFactory::getDbo();
		$query = 'SELECT
					p.`product_length`,
					p.`product_width`,
					...
					pr.`product_price`
				FROM
					`#__virtuemart_products` p
				INNER JOIN `#__virtuemart_product_prices` pr ON p.`virtuemart_product_id` = pr.`virtuemart_product_id`
				WHERE
					p.`virtuemart_product_id` = ' . $parent_product_id . ' AND p.`published` = 1';
		$db->setQuery($query);
		return $db->loadAssoc();
	}
}
