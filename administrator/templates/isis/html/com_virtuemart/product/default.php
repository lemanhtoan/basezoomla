...
?>
	...
	<tr>
		...
		<?php if (!$product_parent_id ) { ?>
                <th width="15%"><?php echo $this->sort('product_parent_id','COM_VIRTUEMART_PRODUCT_CHILDREN_OF'); ?></th>
                <?php } ?>
                <th width="15%" ><?php echo vmText::_('COM_VIRTUEMART_PRODUCT_PARENT_LIST_CHILDREN'); ?></th>
		...
		<th width="10%" ><?php echo $this->sort('product_price', 'COM_VIRTUEMART_PRODUCT_PRICE_TITLE') ; ?></th>
...
<th width="20%"><?php echo vmText::_( 'COM_VIRTUEMART_CATEGORY'); ?></th>





		<!-- Only show reordering fields when a category ID is selected! TVX -->
		<?php
		$num_rows = 0;
		if( $this->virtuemart_category_id && !$product_parent_id) { ?>
			<th style="min-width:100px;width:5%;">
				<?php
					//echo $this->sort('pc.ordering', 'COM_VIRTUEMART_FIELDMANAGER_REORDER'); TVX COMMENT 
				?>
	...
	if ($totalList = count($this->productlist) ) {
		$i = 0;
		$k = 0;
		$keyword = vRequest::getCmd('keyword');  //echo "<pre>"; var_dump($this->productlist); die;
		foreach ($this->productlist as $key => $product) {
			...
			<tr class="row<?php echo $k ; ?>">
				...
				<!-- Product price -->
				<td align="right" ><?php 
                                    if(isset($product->allPrices['0']['product_price'])) {
                                        if ($product->allPrices['0']['product_price'] != "") {
                                            echo number_format($product->allPrices['0']['product_price'], 2) . $currencySymbol;
					} else {
                                            echo "No price set";
                                        }
                                    }
				...
				?></td>
				<!-- Reorder only when category ID is present TVX tvx-->
				<?php if ($this->virtuemart_category_id && !$product_parent_id) { ?>
					...
					<td class="order" >
						...
						<input class="ordering" type="text" name="order[<?php echo $product->id?>]" id="order[<?php echo $i?>]" size="5" value="<?php echo $product->ordering; ?>" style="text-align: center; display: none;" />
