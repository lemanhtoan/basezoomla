...
class JdDatabase extends JdDatabaseAdapter {
	...
	/** @var array Temporary stored queries */
	...
	public function loadObject($className = 'stdClass') {
		if (!$this->translateQuery()) {
			$result = parent::loadObject($className);
			return $result;
		}
	...
	public function loadObjectList($key = null, $className = 'stdClass') {
		if (!$this->translateQuery()) {
			$result = parent::loadObjectList($key, $className);
			return $result;
		}
	...
	protected function translateQuery() {
		...
		if (!empty($_REQUEST['jDictionDebug'])) JLog::add('Query: ' . str_replace("\n", '\n', $this->sql), JLog::DEBUG, 'jdiction');
		if (!empty($_REQUEST['jDictionDebug']) && $_REQUEST['jDictionDebug'] > 1) JLog::add('Backtrace: ' . @json_encode(debug_backtrace()), JLog::DEBUG, 'jdiction');

		// We don't translate #__session so we don't need to initialise jDiction at this point
		/* not needed because we don't run before languagefilter is ready
		if (strpos($this->sql, '#__session') !== false) {
			return $translate;
		} */

		$this->jd->initialise();

		if ($this->jd_active && $this->jd->getStatus()
			...
			if (!empty($_REQUEST['jDictionDebug'])) JLog::add('Query translated: ' . str_replace("\n", '\n', $this->sql), JLog::DEBUG, 'jdiction');
		} else {
			if (!empty($_REQUEST['jDictionDebug'])) JLog::add('Query not translated: ' . str_replace("\n", '\n', $this->sql), JLog::DEBUG, 'jdiction');
		}
		return $translate;
	}
 ...
 */
	...
	/**
	 * Copies the current query to stash
	 */
	public function stashQuery() {
		array_unshift($this->stash, $this->sql);
	}

	/**
	 * moves the last query from the stash to the active current query
	 */
	public function unstashQuery() {
		$this->sql = array_shift($this->stash);
	}
