...
class jDiction extends JObject {
	...
	/**
	 * defines if jDiction is booting
	 * @var bool
	 */
	...
	public function initialise() {
		...
		// We only translate in frontend and need the languagefilter plugin loaded, we may get called before Joomla is ready
		if (empty(JFactory::$application)) {
			return;
		}

		// There is no getLanguageFilter function in Backend
		if (is_a(JFactory::$application, 'JApplicationSite') && !JFactory::getApplication()->getLanguageFilter()) {
			return;
		}

		// Check if we get called twice
	...
	public function WorkaroundDatabaseConfig() {
		...
		// We can't use Joomla\Input\Input here because JApplication is not ready yet
		if (!empty($_REQUEST['option']) && $_REQUEST['option'] == 'com_config') {
    ...
    $subdirs = $cache->get(array(get_class($this), 'findTables'));
    ...
    $component = (string)$xml->component;
      ...
      foreach($section->views->children() as $view) {
        ...
        $this->components[$component][(string)$view['name']]->form = simplexml_load_string($view->form->asXML());
				...
				if ((string)$view->list['name'] != '') {
					$this->components[$component][(string)$view['name']]->list = (string)$view->list['name'];
				}

				$this->components[$component][(string)$view['name']]->listview = new stdClass;
				$this->components[$component][(string)$view['name']]->listview->tableselector = '.table';

				if ($view->list->count() > 0) {
					if ((string)$view->list->tableselector != '') {
						$this->components[$component][(string)$view['name']]->listview->tableselector = (string)$view->list->tableselector;
						$this->components[$component][(string)$view['name']]->listview->columnselector = (string)$view->list->columnselector;
					}
				}
      }
 ...
 */
   ...
   * @return array All views or all components
   */
  public function getViewByTask($component, $task, $controller=null) {
    if (isset($this->components[$component])) {
      foreach($this->components[$component] as $jdview) {
        if (
          (!empty($controller) && $jdview->name == $controller.'.'.$task) ||
          (!empty($controller) && $jdview->list == $controller.'.'.$task) ||
          $jdview->name == $task ||
          $jdview->list == $task ||
          ($jdview->default == 'true' && $task=='default')
        ) {
          return $jdview;
        }
      }
  ...
  public function getCurrentLanguage() {
	...
	/**
   * @return mixed the default Language
   */
  public function getDefaultLanguage() {
    $lang = $this->getLanguage(JComponentHelper::getParams('com_languages')->get('site', 'en-GB'));
    return current($lang);
  }
    ...
    if ($view || !$task) {
      ...
      $viewobj = $this->getViewByTask($option, $task, $controller);
    } else {
      return;
    }
	...
	public function getTranslationLink($component, $view, $layout, $id, $popup=true , $language_tag) {
		...
		$link .= '&jd_option='.$component.'&jd_view='.$view.'&jd_layout='.$layout.'&jd_id='.$id.'#language-tab-'.$language_tag;
		//var_dump($link);
		return $link;
	}
}