/*
jquery.labelOver
DWS: allows you to put a label over a text-input
*/
		...
		function()
		{
			...
			function buildDropDownList()
			{
				...
				else
				{
					...
					_dropDownListJQuery.appendTo(document.body);
...
//					var selectedText = originalElement[originalElement.selectedIndex].text;
					var selectedText = originalElement[originalElement.selectedIndex].getAttribute('displaytext');
					//displaytext
					//<span class="comboboxPreTitle">Quick search:&nbsp;</span>
					_valueDisplayJQuery.html('<span class="comboboxPreTitle">Quick search:&nbsp;</span><span class="comboboxSearchTitle">'+selectedText+'</span>');
					//_valueDisplayJQuery.attr("title", selectedText); DWS: we do not want this set
			...
			function toggleDropDownList(isShown)
			{
				if (isShown)
				{
					if (_dropDownListJQuery.is(":hidden"))
					{
						...
						_dropDownListJQuery.css("top", dropdownListTop+4);
						_dropDownListJQuery.css("left", _containerJQuery.offset().left+1);//dws: hack to make it look better
			...
			_originalElementJQuery.combobox.updateWidth =
				function(nx)
				{
					//alert('no');
					applyLayout();
//					_containerJQuery.width(nx);
//					updateDropDownListWidth();
				};			
