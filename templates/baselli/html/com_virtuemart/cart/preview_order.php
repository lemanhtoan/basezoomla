...
<div class="container order_page order_page_preview">
	<div class="row">
		...
		<div class="myorder_grid_container mobile_version">
			...
			<div class="myorder_grid_mobile_row">
				<div class="myorder_grid_mobile_col1"><?php echo JText::_('Name') ?></div>
				<div class="myorder_grid_mobile_col2"><a class="product_link" href="<?php echo JRoute::_($item->link) ?>"><?php echo $product_name ?></a></div>
			</div>
			<div class="myorder_grid_mobile_row">
				<div class="myorder_grid_mobile_col1"><?php echo JText::_('Color') ?></div>
			...
			<div class="myorder_grid_mobile_row">
				...
				<div class="myorder_grid_mobile_col2"><?php echo $item->quantity ?></div>
			</div>
			<div class="myorder_grid_mobile_row myorder_grid_mobile_total_row">
		...
		<table class="myorder_grid desktop_version" id="myorder_grid">
			...
			<tr class="myorder_grid_first_row myorder_grid_row">
				<td class="col_1"><?php echo $i;//$item->virtuemart_product_id ?></td>
				<td class="col_2">
				...
				<td class="col_5">
					<?php //echo $this->currencyDisplay->createPriceDiv ('product_price', '', $item->prices, TRUE, FALSE) ?>
					<?php echo number_format($product_price, 2) . ' ', $currencySymbol; //$currencySymbol,  ?>
				</td>
				<td class="col_6">
					<div class="quantity_container" style="border-bottom:0;width:auto;">
						<?php echo $item->quantity ?>

						<span class="quantity_container_subtract" style="display:none"></span>
						<input style="display:none" data-submit-id="updatecart_<?php echo ($i  - 1); ?>" readonly="readonly" type="text" value="<?php echo $item->quantity ?>" name="quantity[<?php echo ($i  - 1); ?>]"/> 
						<span style="display:none" class="quantity_container_plus"></span>
					</div>
				</td>
				<td class="col_7"><?php echo number_format($product_price*$item->quantity, 2)  . ' ', $currencySymbol; //$currencySymbol,  ?></td>
			...
			<tr class="myorder_grid_last_row myorder_grid_row">
				<td colspan="6"></td>	
				<td><strong class="total_price"><?php echo number_format($totalPrice, 2)  . ' ', $currencySymbol; //$currencySymbol, ?></strong></td>	
				
			</tr>
			-->

			<tr class="">
				<td colspan="5" style="border-top:1px solid #DADADA;padding-top:25px;"></td>	
				<td style="font-size:16px;border-bottom:0px solid #DADADA;text-align:right;padding-right:0px;padding-bottom:15px;border-top:1px solid #DADADA;padding-top:25px;"><?php echo JText::_('SUB_TOTAL') ?></td>	
				<td style="font-size:16px;border-bottom:0px solid #DADADA;padding-bottom:15px;text-align:center;border-top:1px solid #DADADA;padding-top:25px;"><strong><?php echo number_format($totalPrice, 2) . ' ', $currencySymbol; //$currencySymbol,  ?></strong></td>	
			</tr>
            
            <?php if ($check_user_group == "user_opticoach") : ?>
            <tr class="">
				<td colspan="5" style="border-top:1px solid #DADADA;padding-top:25px;"></td>	
				<td style="font-size:16px;border-bottom:0px solid #DADADA;text-align:right;padding-right:0px;padding-bottom:15px;border-top:1px solid #DADADA;padding-top:25px;"><?php echo JText::_('DISCOUNT'), ' ', ceil($tax), '%' ?></td>	
				<td style="font-size:16px;border-bottom:0px solid #DADADA;padding-bottom:15px;text-align:center;border-top:1px solid #DADADA;padding-top:25px;"><strong><?php echo number_format($totalPrice*($tax/100), 2) . ' ', $currencySymbol; ?></strong></td>	
			</tr>
			...
			<tr class="">
				<td colspan="5"></td>	
				<td style="font-size:16px;border-bottom:0px solid #DADADA;text-align:right;padding-right:0px;padding-bottom:15px;"><?php echo JText::_('VAT'), ' ', ceil($tax), '%' ?></td>	
				<td style="font-size:16px;border-bottom:0px solid #DADADA;padding-bottom:15px;text-align:center;"><strong>
                <?php 
                    if ($check_user_group == "user_opticoach") {
                        $total_discount = $totalPrice - $totalPrice*floatval($tax/100);
                        echo sprintf("%.2f",number_format($total_discount*($tax/100), 2)) .  $currencySymbol;
			...
			<tr class="">
				<td colspan="5"></td>	
				<td style="font-size:18px;border-top:1px solid #DADADA;text-transform:uppercase;text-align:right;padding-right:0px;padding-top:15px;"><?php echo JText::_('GRAND_TOTAL') ?></td>	
				<td style="font-size:18px;border-top:1px solid #DADADA;padding-top:15px;text-align:center;"><strong>
                    <?php 
                        if ($check_user_group == "user_opticoach") {
                            $total_discount = $totalPrice - $totalPrice*floatval($tax/100);
                            $total_final = $total_discount + $total_discount*($tax/100);
		...
		<?php if($user): ?>
			<div class="row mycart_sys_buttons">
				<div class="col-sm-12 col-xs-12 col-md-12 col-lg-5 pull-right" style="text-align:right;">
					<button class="continue_shopping" onclick="window.location.href='<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=cart' . $taskRoute, $this->useXHTML, $this->useSSL) ?>'; return false;"><?php echo JText::_('BACK_TO_EDIT_ORDER') ?></button>
					<button class="preview_order"><?php echo JText::_('SUBMIT_ORDER') ?></button>
				</div>
			</div>
		<?php endif; ?>
	</div>
	...
	<div class="row">
		<div class="additional_block">
			...
			<textarea style="border:none;border-bottom:1px solid #dadada;width:568px;height:40px;display:none;" id="additional_information" name="additional_information"><?php echo $session->get('additional_information') ?></textarea>
			<p style="font-style:normal;"><?php echo $session->get('additional_information') ?></p>
		</div>
	</div>
...
<script type="text/javascript">
	jQuery(document).ready(function(){
		...
		jQuery('.preview_order').click(function(){
